...
impl Analyzer {
    pub fn new(recv: Receiver<Mesurement>) -> thread::JoinHandle<Analyzer> {
        thread::spawn(move || {
            ...
            while let Ok(measurement) = recv.recv() {
                let pid = measurement.pid;
    
                analyzer.analyze.entry(pid)
                .or_insert_with(|| Analyze::new(pid)).add(measurement);
            }
            analyzer
        })
    }
    ...
    pub fn print(mut self) -> String {
        let mut output= String::from("  PID |Core|    Runtime    | Context \n------|----|---------------|-----------------------\n");

        for (pid, anna) in self.analyze().unwrap().analyze.iter() {
            for (src, time_vec) in anna.runtime_expression.iter() {
                for (start_time_ns, time_ns) in time_vec {
                    for (core_start,core) in anna.core_affinity.iter() {
                        if core_start == start_time_ns {           
                            output = format!("{}{:>5} | {:>2} | {:>10} ns | Line[{:^4}]: {}\n",output,pid,core,time_ns,src.line,src.expression.lines().next().unwrap());
                            break;
                        }
                    }
                }
